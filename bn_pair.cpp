...
//Big Zp
Big PFC::Zpadd(Big &a, Big &b)
{
    Big z=a+b;
    return (z)%(*ord);
}
Big PFC::Zpmulti(Big &a, Big &b)
{
    Big z=a*b;
    return (z)%(*ord);
}
Big PFC::Zpsub(Big &a, Big &b)
{
    Big z=a-b;
    return (z)%(*ord);
}
Big PFC::Zpinverse(Big &x)//shir
{
    Big z;
   // z=-x;
    xgcd(x.fn,(*ord).fn,z.fn,z.fn,z.fn);
#if 0
    Big t;
    Big y=1;
    t=(x*z)%(*ord);
    if(t==1)
            printf("modinverse OK\n");
    else
            printf("modinverse erro\n");
#endif
    return z;
}
